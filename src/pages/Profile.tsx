...
export const Profile = () => {
  ...
  // If onboarding and user already has credentials, redirect to dashboard
  useEffect(() => {
    if (isOnboarding && hasAlpacaCredentials && !setupComplete) {
      navigate('/');
    }
  }, [isOnboarding, hasAlpacaCredentials, navigate, setupComplete]);
  ...
  const handleSaveAlpacaCredentials = async () => {
    ...
    if (result.error) {
      ...
      toast.success('Alpaca credentials saved securely!');
      setAlpacaApiKey('');
      setAlpacaSecretKey('');
      if (isOnboarding) {
  ...
  const currentStep = setupComplete ? 3 : hasAlpacaCredentials ? 3 : 2;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border/50 bg-card/50 backdrop-blur sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold">{isOnboarding ? 'Welcome to TrAide' : 'Settings'}</h1>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        {isOnboarding && !setupComplete && (
          <div className="glass-card rounded-2xl p-6 border-primary/30">
            ...
            <p className="text-sm text-muted-foreground mb-5">
              Connect your Alpaca brokerage account to start trading with Trai.
            </p>
            {/* Step Indicators */}
            <div className="flex items-center gap-3">
              {[
                { num: 1, label: 'Create Account' },
                { num: 2, label: 'Connect Alpaca' },
                { num: 3, label: 'Start Trading' },
              ].map((s, i) => (
                <div key={s.num} className="flex items-center gap-3 flex-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                      ...
                      {s.num < currentStep ? <CheckCircle2 className="h-4 w-4" /> : s.num}
                    </div>
                  ...
                  {i < 2 && <div className="h-px flex-1 bg-border/60" />}
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Setup Complete */}
        {isOnboarding && setupComplete && (
          <div className="glass-card rounded-2xl p-8 text-center border-success/30">
            ...
            <p className="text-muted-foreground mb-6">
              Your Alpaca account is connected. Trai is ready to help you trade.
            </p>
            <Button size="lg" onClick={() => navigate('/')} className="glow-primary">
              Go to Dashboard
        ...
        {/* Profile Card — hide during onboarding to keep focus */}
        {!isOnboarding && (
            ...
            <CardContent className="space-y-6">
              <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-muted-foreground">Your API keys are encrypted before being stored and can only be decrypted server-side.</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-secondary/30 rounded-xl">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Use paper trading for risk-free practice</p>
                </div>
                <Switch id="paperTrading" checked={paperTrading} onCheckedChange={setPaperTrading} />
              </div>
              ...
              {!paperTrading && (
                <div className="flex items-start gap-3 p-4 bg-destructive/10 border border-destructive/30 rounded-xl">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-muted-foreground">Real money will be used for trades.</p>
                  </div>
                </div>
              )}
