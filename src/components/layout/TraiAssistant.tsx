...
const OrderStatusBadge = ({ pendingOrder, orderStatus }: OrderStatusBadgeProps) => {
  ...
  if (orderStatus === 'submitting') {
    statusConfig = { icon: Loader2, text: 'Submitting...', className: 'bg-primary/20 text-primary border-primary/30' };
  } else if (orderStatus === 'confirmed') {
    statusConfig = { icon: CheckCircle2, text: 'Confirmed!', className: 'bg-success/20 text-success border-success/30' };
  } else if (orderStatus === 'failed') {
    statusConfig = { icon: AlertCircle, text: 'Failed', className: 'bg-destructive/20 text-destructive border-destructive/30' };
  }

  const Icon = statusConfig.icon;
  return (
    <div className={cn("inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-medium border", statusConfig.className)}>
...
const ChatContent = ({
  messages, pendingOrder, watchlist, isLoading, orderStatus,
  quickActions, input, onInputChange, onSubmit, onQuickAction,
  onConfirmOrder, onCancelOrder, chatInputRef,
  ...
  return (
    <div className={cn("flex flex-col", className)} style={{ minHeight: 0 }}>
        ...
        >
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center p-4" style={{ minHeight: '100%' }}>
              <div className="w-12 h-12 rounded-2xl gradient-purple glow-primary flex items-center justify-center mb-3">
                <Sparkles className="h-6 w-6 text-white" />
              </div>
              <h3 className="font-bold text-sm text-foreground mb-1.5">Hey! I'm Trai</h3>
              <p className="text-[11px] text-muted-foreground mb-4 max-w-[240px]">
                Your AI trading companion. I can analyze markets, suggest trades, and help you make smarter decisions.
              </p>
              <div className="w-full space-y-1.5 max-w-[260px]">
                {quickActions.map((qa) => (
                  <button
                    key={qa.label}
              ...
              {messages.map((message) => (
                <div key={message.id} className={cn('flex gap-2', message.role === 'user' ? 'flex-row-reverse' : '')}>
                  <div className={cn(
                    ...
                    {message.role === 'assistant' ? <Bot className="h-3.5 w-3.5 text-white" /> : <User className="h-3.5 w-3.5 text-primary" />}
                  </div>
                  <div className={cn('max-w-[85%]', message.role === 'assistant' ? 'chat-bubble-ai' : 'chat-bubble-user')}>
                      ...
                      message.content ? (
                        ...
                        <span className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">
                            {thinkingElapsed < 5 ? 'Thinking...' : thinkingElapsed < 15 ? 'Analyzing markets...' : thinkingElapsed < 30 ? 'Still working on it...' : 'Taking longer than usual — hang tight'}
                          </span>
                        </span>
                      ) : null
                    ) : (
      ...
      {/* Preview action */}
      {previewAction && onConfirmPreview && onCancelPreview && (
        <div className="shrink-0 px-3 py-2.5 border-t border-border/30 bg-secondary/30">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={onCancelPreview} className="text-[10px] h-7 rounded-lg flex-1">Cancel</Button>
      ...
      {messages.length > 0 && !previewAction && (
        <div className="shrink-0 px-3 py-2 border-t border-border/30 flex gap-1.5 overflow-x-auto scrollbar-thin">
          {quickActions.map((qa) => (
            <button
              key={qa.label}
              onClick={() => onQuickAction(qa.action)}
      ...
      {/* Input */}
      <div className="shrink-0 p-3 border-t border-border/30 bg-card/50">
        <form onSubmit={onSubmit} className="flex gap-2">
          <Input
            ...
            enterKeyHint="send"
            autoComplete="off"
            className="flex-1 h-9 bg-input border-border/50 rounded-xl text-sm"
            disabled={isLoading}
          />
          <Button 
            type="submit" 
...
export const TraiAssistant = ({
  messages, pendingOrder, watchlist, positions, isLoading,
  currentTab, currentSymbol, orderStatus,
  onSendMessage, onConfirmOrder, onCancelOrder, onNavigate,
  isFullPage = false, isSidebar = false
}: TraiAssistantProps) => {
  ...
  const quickActions = useMemo((): QuickAction[] => {
    if (pendingOrder) {
      return [
        ...
        { label: 'Adjust', action: 'Adjust the order before I confirm.', icon: Settings, variant: 'primary' },
      ];
    }
    ...
    switch (currentTab) {
      ...
      case 'portfolio':
        if (positions.length > 0) {
          base.push({ label: `Analyze ${positions[0].symbol}`, action: `Analyze my ${positions[0].symbol} position`, icon: Eye, variant: 'primary' });
        }
        base.push(
      ...
      case 'options':
        ...
        base.push({ label: 'IV insights', action: 'Where is implied volatility high right now?', icon: Eye, variant: 'warning' });
        break;
      case 'history':
        base.push(
  ...
  const handleQuickAction = useCallback((action: string) => {
    if (action === '__confirm_order__') { onConfirmOrder(); return; }
    if (action === '__cancel_order__') { onCancelOrder(); return; }
    setPreviewAction(action);
    if (expansionState === 'collapsed' && !isFullPage) setExpansionState('full');
  }, [onConfirmOrder, onCancelOrder, expansionState, isFullPage]);

  const confirmPreviewAction = useCallback(() => {
    if (previewAction) { onSendMessage(previewAction); setPreviewAction(null); }
  }, [previewAction, onSendMessage]);

  const cancelPreviewAction = useCallback(() => setPreviewAction(null), []);

  const chatContentProps = {
    messages, pendingOrder, watchlist, isLoading, orderStatus, quickActions,
    input, onInputChange: setInput, onSubmit: handleSubmit, onQuickAction: handleQuickAction,
    onConfirmOrder, onCancelOrder, chatInputRef,
    previewAction, onConfirmPreview: confirmPreviewAction, onCancelPreview: cancelPreviewAction,
  };
  ...
  if (isFullPage) {
    return (
      <div className="h-full flex flex-col bg-background overflow-hidden">
        ...
        <div className="flex items-center justify-between p-3 border-b border-border/30 bg-gradient-to-r from-primary/5 to-transparent shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl gradient-purple glow-primary flex items-center justify-center">
              <Sparkles className="h-5 w-5 text-white" />
            </div>
            <div>
              <h1 className="font-bold text-base text-foreground">Trai</h1>
  ...
  // ── Sidebar mode (desktop right panel) ──
  if (isSidebar) {
    if (!sidebarOpen) {
      return (
        <div className="h-full flex flex-col items-center border-l border-border/30 bg-card/30 w-12">
          <button
            ...
            className="mt-3 w-9 h-9 rounded-xl gradient-purple flex items-center justify-center hover:scale-105 transition-transform"
            title="Open Trai"
            aria-label="Open Trai assistant"
          >
            <Sparkles className="h-4 w-4 text-white" />
          </button>
          {pendingOrder && <span className="mt-2 w-3 h-3 bg-warning rounded-full animate-pulse" />}
        </div>
      );
    }

    return (
      <div className="h-full flex flex-col bg-card/30 overflow-hidden w-[320px] xl:w-[360px] border-l border-border/30">
        <ConfirmationToast show={showConfirmAnimation} />
        <div className="flex items-center justify-between p-3 border-b border-border/30 bg-gradient-to-r from-primary/5 to-transparent shrink-0">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              title="Collapse Trai panel"
              aria-label="Collapse Trai panel"
            >
        ...
        {pendingOrder && (
          <div className="p-2 bg-warning/10 border-b border-warning/30 flex items-center gap-2 shrink-0">
    ...
    return (
      <>
        ...
        >
          <Sparkles className="h-5 w-5 text-white" />
          {pendingOrder && (
            <span className="absolute -top-1 -right-1 w-4 h-4 bg-warning text-warning-foreground text-[10px] rounded-full flex items-center justify-center animate-pulse">!</span>
          )}
        </button>
      </>
    );
          ...
          <div className="pointer-events-auto glass-card rounded-xl border border-primary/20 shadow-xl overflow-hidden">
            ...
            {pendingOrder && (
              <div className="p-2 bg-warning/10 border-b border-warning/30 flex items-center gap-2">
                ...
                <Button size="sm" variant="outline" className="ml-auto h-6 text-[10px] px-2" onClick={() => setExpansionState('full')}>
                  Review
                </Button>
              </div>
            )}
            ...
            <div className="p-2 flex gap-1.5 overflow-x-auto scrollbar-thin">
              {quickActions.map((qa) => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-[10px] font-medium whitespace-nowrap border bg-secondary/50 hover:bg-secondary",
                    qa.variant === 'primary' && "border-primary/30 text-primary",
                    qa.variant === 'success' && "border-success/30 text-success",
                    qa.variant === 'warning' && "border-warning/30 text-warning"
                  )}
            ...
            <div className="mx-2 mb-2">
              <form onSubmit={(e) => { e.preventDefault(); setExpansionState('full'); }} className="flex gap-2">
                <Input value="" readOnly onFocus={() => setExpansionState('full')} placeholder="Message Trai..." className="flex-1 h-9 bg-input/50 border-border/50 rounded-xl text-sm cursor-pointer" />
                <Button type="button" size="icon" onClick={() => setExpansionState('full')} className="h-9 w-9 rounded-xl bg-primary hover:bg-primary/90">
  ...
  // ── Full expanded (mobile overlay) ──
  return (
    <>
      <ConfirmationToast show={showConfirmAnimation} />
      <div className="fixed inset-0 z-50 md:inset-auto md:bottom-4 md:right-4 md:w-[400px] md:h-[550px]">
        <div className="h-full glass-card md:rounded-2xl border-t md:border border-primary/20 shadow-2xl flex flex-col overflow-hidden bg-background/95 backdrop-blur-xl" style={{ height: '100dvh', maxHeight: '100dvh' }}>
          <div className="flex items-center justify-between p-3 bg-gradient-to-r from-primary/10 to-transparent border-b border-border/30 shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg gradient-purple flex items-center justify-center">
                <Sparkles className="h-4 w-4 text-white" />
              </div>
            ...
            <div className="flex items-center gap-1">
              <OrderStatusBadge pendingOrder={pendingOrder} orderStatus={orderStatus} />
              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => setExpansionState('collapsed')}>
                <X className="h-4 w-4" />
              </Button>
            </div>
          </div>
